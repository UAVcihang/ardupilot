...
AC_Flowermeter_Pin::IrqState AC_Flowermeter_Pin::irq_state[2];
...
void AC_Flowermeter_Pin::irq_handler(uint8_t instance)
{
 ...
 */
void AC_Flowermeter_Pin::irq_handler0(void)
{
    irq_handler(0);
}
...
void AC_Flowermeter_Pin::update(void)
{
    if (last_pin != get_pin()) {
        ...
        stm32_gpiosetevent(gpio, true, false, false,
                           irq_handler);
#else // other HALs
        hal.gpio->attach_interrupt(last_pin, irq_handler0,HAL_GPIO_INTERRUPT_RISING);
#endif
    }

	//uint32_t now = AP_HAL::micros();
	if(AP_HAL::micros() - irq_state[0].last_pulse_us < 1000000) // 1s
	{
		_state._period = irq_state[0].dt_period;
		_state._vel_flower = irq_state[0].dt_count - _state._count;
		_state._count = irq_state[0].dt_count;
