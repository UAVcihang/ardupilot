...
/* Add by weihli */
/// update_zigzag_wpnav - run the zigzag mode wp controller - throttle can be controlled
bool AC_WPNav::update_zigzag_wpnav()
{
    // calculate dt
    float dt = _pos_control.time_since_last_xy_update();
    bool ret = true;
...
/// advance_wp_target_along_track_xy - move target location along track from origin to destination except z axis
    ...
    if( !_flags.reached_destination ) {
        if( _track_desired >= _track_length ) {
            ...
            if (_flags.fast_waypoint) {
                ...
                // regular waypoints also require the copter to be within the waypoint radius
                Vector3f dist_to_dest = (curr_pos - Vector3f(0,0,terr_offset)) - _destination;
                if( /*dist_to_dest.length() <= _wp_radius_cm*/norm(dist_to_dest.x, dist_to_dest.y) <= _wp_radius_cm ) { // comment by weihli for zigzag remove z
                    _flags.reached_destination = true;
                }
            }
        }
