...
// add by weihli
/* MQTT servers connection state */
enum connectState { CONNECT_NONE= 0, CONNECT_SEND, CONNECT_SEND_TIMEOUT, CONNECT_RECEIVE, CONNECT_RECEIVE_TIMEOUT, CONNECT_OK};
...
public:
     ...
     */
    Client(Network& network, unsigned int _command_timeout_ms = 30000);
...
private:
    ...
    // add by weihli
    int read_nb(int packet_type, Timer& timer);
...
template<class Network, class Timer, int a, int MAX_MESSAGE_HANDLERS>
MQTT::Client<Network, Timer, a, MAX_MESSAGE_HANDLERS>::Client(Network& network, unsigned int _command_timeout_ms)  : ipstack(network), packetid()
{
    this->command_timeout_ms = _command_timeout_ms;
    cleansession = true;
	  closeSession();
}
...
int MQTT::Client<Network, Timer, a, b>::sendPacket(int length, Timer& timer)
{
    ...
    if (sent == length)
    {
        ...
        rc = SUCCESS;
        conctState = CONNECT_SEND;
    }
    else
    {
        rc = FAILURE;
        conctState = CONNECT_SEND_TIMEOUT;
    }
...
// read non block
template<class Network, class Timer, int a, int b>
int MQTT::Client<Network, Timer, a, b>::read_nb(int packet_type, Timer& timer)
{
	int rc = FAILURE;
