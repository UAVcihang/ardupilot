...
//                	 hal.console->printf("SerialProtocol_NRA24 \n\r");
                     // Note baudrate is hardcoded to 115200
                 ...
                 case SerialProtocol_HPS166U:                        //避障使用，HPS166U
//                	 hal.console->printf("SerialProtocol_HPS166U \n\r");
                     // Note baudrate is hardcoded to 115200
                     state[i].baud = AP_SERIALMANAGER_ULANDING_BAUD / 1000;   // update baud param in case user looks at it
                     state[i].uart->begin(map_baudrate(state[i].baud),
                                          AP_SERIALMANAGER_ULANDING_BUFSIZE_RX,
                                          AP_SERIALMANAGER_ULANDING_BUFSIZE_TX);
                 	break;
                 case SerialProtocol_HPS167UL:                     //避障使用，HPS167U
//                	 hal.console->printf("SerialProtocol_HPS167UL \n\r");
                     // Note baudrate is hardcoded to 115200
                     state[i].baud = AP_SERIALMANAGER_ULANDING_BAUD / 1000;   // update baud param in case user looks at it
                     state[i].uart->begin(map_baudrate(state[i].baud),
                                          AP_SERIALMANAGER_ULANDING_BUFSIZE_RX,
