...
/*void AC_UKF::altUkfTimeUpdate(float *in, float *noise, float *out, float *u, float dt, int n) {
    float acc;
    int i;
...
/*void AC_UKF::altUkfPresUpdate(float *u, float *x, float *noise, float *y) {
    y[0] = x[ALT_STATE_POS] + noise[0];     // return altitude
}*/

void AC_UKF::init(void){

    float acc[3], mag[3];
    //float pres;
    int i;

    memset((void *)&_runData, 0, sizeof(_runData));

	Vector3f mag_field = _ahrs->get_compass()->get_field() * 0.001f;
	mag[0] = mag_field.x;
	mag[1] = mag_field.y;
	mag[2] = mag_field.z;
	/*mag[0] = IMU_MAGX;
    ...
    Vector3f accel = _ahrs->get_ins().get_accel();
    acc[0] = accel.x;
    acc[1] = accel.y;
    acc[2] = accel.z;
    /*acc[0] = IMU_ACCX;
    ...
    for (i = 0; i < RUN_SENSOR_HIST; i++) {
        _runData.accHist[0][i] = acc[0];
        _runData.accHist[1][i] = acc[1];
        _runData.accHist[2][i] = acc[2];
        _runData.magHist[0][i] = mag[0];
        _runData.magHist[1][i] = mag[1];
        _runData.magHist[2][i] = mag[2];


        _runData.sumAcc[0] += acc[0];
        _runData.sumAcc[1] += acc[1];
        _runData.sumAcc[2] += acc[2];
        _runData.sumMag[0] += mag[0];
        _runData.sumMag[1] += mag[1];
        _runData.sumMag[2] += mag[2];

    }

    _runData.sensorHistIndex = 0;

    navUkfInit();
}

void AC_UKF::navUkfInit()
{
    float Q[SIM_S];		// state variance
    float V[SIM_V];		// process variance
    float mag[3];

    //memset((void *)&_navUkfData, 0, sizeof(_navUkfData));
    ...
    mag[0] = cosf(radians(IMU_MAG_INCL));
    mag[1] = 0.0f;
    mag[2] = -sinf(radians(IMU_MAG_INCL));

    // rotate local mag vector to align with true north
    _v0m[0] = mag[0] * cosf(radians(IMU_MAG_DECL)) - mag[1] * sinf(radians(IMU_MAG_DECL));
    _v0m[1] = mag[1] * cosf(radians(IMU_MAG_DECL)) + mag[0] * sinf(radians(IMU_MAG_DECL));
    _v0m[2] = mag[2];

    kf->srcdkfInit(SIM_S, SIM_M, SIM_V, SIM_N);
    //_navUkfData.kf = srcdkfInit(SIM_S, SIM_M, SIM_V, SIM_N, navUkfTimeUpdate);

    _x = kf->srcdkfGetState();
    ...
    //Q[4] = UKF_POS_Q;
    //Q[5] = UKF_POS_ALT_Q;
    Q[UKF_STATE_ACC_BIAS_X] = UKF_ACC_BIAS_Q;
    Q[UKF_STATE_ACC_BIAS_Y] = UKF_ACC_BIAS_Q;
    Q[UKF_STATE_ACC_BIAS_Z] = UKF_ACC_BIAS_Q;
    Q[UKF_STATE_GYO_BIAS_X] = UKF_GYO_BIAS_Q;
    Q[UKF_STATE_GYO_BIAS_Y] = UKF_GYO_BIAS_Q;
    Q[UKF_STATE_GYO_BIAS_Z] = UKF_GYO_BIAS_Q;
    Q[UKF_STATE_Q1] = UKF_QUAT_Q;
    Q[UKF_STATE_Q2] = UKF_QUAT_Q;
    Q[UKF_STATE_Q3] = UKF_QUAT_Q;
    Q[UKF_STATE_Q4] = UKF_QUAT_Q;
    //Q[16] = UKF_PRES_ALT_Q;
...
/*bool AC_UKF::check_baro_ukf()
{
    ...
    if( baro_update_time != _baro_last_update ) {
        ...
        _baro_alt = -_baro.get_altitude();

        //correct_with_baro(baro_alt, dt);
...
/*void AC_UKF::altDoPresUpdate() {
    float noise;        // measurement variance
    float y;            // measurment

    noise = ALT_PRES_NOISE;
    if(check_baro_ukf()){
    y = _baro_alt;//navUkfPresToAlt(measuredPres);

    srcdkfMeasurementUpdate(_altUkfData.kf, 0, &y, 1, 1, &noise, altUkfPresUpdate);
    }
}*/
...
void AC_UKF::update()
{
 ...
 /*   float acc[3];
    ...
    Vector3f accel = _ahrs->get_ins().get_accel();
    Vector3f accel_ef = _ahrs->get_rotation_body_to_ned() * _ahrs->get_rotation_autopilot_body_to_vehicle_body() * accel;
    ...
    acc[2] = accel_ef.z + GRAVITY_MSS;

    srcdkfTimeUpdate(_altUkfData.kf, &acc[2], _dt);
	...
	//hal.console->printf("update begin\n");
	navUkfInertialUpdate(_dt); // 200hz
	//hal.console->printf("update end\n");
	// record history for acc & mag & pressure readings for smoothing purposes
	// acc
	_runData.sumAcc[0] -= _runData.accHist[0][_runData.sensorHistIndex];
	_runData.sumAcc[1] -= _runData.accHist[1][_runData.sensorHistIndex];
	_runData.sumAcc[2] -= _runData.accHist[2][_runData.sensorHistIndex];


    Vector3f accel = _ahrs->get_ins().get_accel();
	_runData.accHist[0][_runData.sensorHistIndex] = accel.x;
	_runData.accHist[1][_runData.sensorHistIndex] = accel.y;
	_runData.accHist[2][_runData.sensorHistIndex] = accel.z;
	...
	// mag
	_runData.sumMag[0] -= _runData.magHist[0][_runData.sensorHistIndex];
	_runData.sumMag[1] -= _runData.magHist[1][_runData.sensorHistIndex];
	_runData.sumMag[2] -= _runData.magHist[2][_runData.sensorHistIndex];

	Vector3f mag_field = _ahrs->get_compass()->get_field() * 0.001f;
	_runData.magHist[0][_runData.sensorHistIndex] = mag_field.x;
	_runData.magHist[1][_runData.sensorHistIndex] = mag_field.y;
	_runData.magHist[2][_runData.sensorHistIndex] = mag_field.z;

	_runData.sumMag[0] += _runData.magHist[0][_runData.sensorHistIndex];
	_runData.sumMag[1] += _runData.magHist[1][_runData.sensorHistIndex];
	_runData.sumMag[2] += _runData.magHist[2][_runData.sensorHistIndex];

	_runData.sensorHistIndex = (_runData.sensorHistIndex + 1) % RUN_SENSOR_HIST;

	if (!((_loops+1) % RUN_SENSOR_HIST)) {
		//#if !Q_FROM_AHRS
	   simDoAccUpdate(_runData.sumAcc[0]*(1.0f / (float)RUN_SENSOR_HIST), _runData.sumAcc[1]*(1.0f / (float)RUN_SENSOR_HIST), _runData.sumAcc[2]*(1.0f / (float)RUN_SENSOR_HIST));
	  //#endif
	}

//#ifndef USE_DIGITAL_IMU
	else if (!((_loops+13) % RUN_SENSOR_HIST)) {
		//#if !Q_FROM_AHRS
	   simDoMagUpdate(_runData.sumMag[0]*(1.0f / (float)RUN_SENSOR_HIST), _runData.sumMag[1]*(1.0f / (float)RUN_SENSOR_HIST), _runData.sumMag[2]*(1.0f / (float)RUN_SENSOR_HIST));
	  //#endif
	}
	...
	// observe that the rates are exactly 0 if not flying or moving
	    ...
	    arm_std_f32(_runData.accHist[0], RUN_SENSOR_HIST, &stdX);
	    arm_std_f32(_runData.accHist[1], RUN_SENSOR_HIST, &stdY);
	    arm_std_f32(_runData.accHist[2], RUN_SENSOR_HIST, &stdZ);

	    Vector3f gyro = _ahrs->get_ins().get_gyro();
	    if ((stdX + stdY + stdZ) < (IMU_STATIC_STD*2)) {
		if (!((_axis + 0) % 3))
		    navUkfZeroRate(gyro.x, 0);
		else if (!((_axis + 1) % 3))
		    navUkfZeroRate(gyro.y, 1);
		else
		    navUkfZeroRate(gyro.z, 2);
		_axis++;
	    }
	}
...
/*void AC_UKF::navUkfCalcEarthRadius(double lat) {
    double sinLat2;

    sinLat2 = sin((double)radians(lat));
    sinLat2 = sinLat2 * sinLat2;

    _navUkfData.r1 = (double)radians(NAV_EQUATORIAL_RADIUS) * ((double)1.0 - (double)NAV_E_2) / pow((double)1.0 - ((double)NAV_E_2 * sinLat2), ((double)3.0 / (double)2.0));
    _navUkfData.r2 = (double)radians(NAV_EQUATORIAL_RADIUS) / sqrt((double)1.0 - ((double)NAV_E_2 * sinLat2)) * cos((double)radians(lat));
}

void AC_UKF::navUkfCalcGlobalDistance(double lat, double lon, float *posNorth, float *posEast) {
    *posNorth = (lat - _navUkfData.holdLat) * _navUkfData.r1;
    *posEast = (lon - _navUkfData.holdLon) * _navUkfData.r2;
}

void AC_UKF::navUkfResetPosition(float deltaN, float deltaE, float deltaD) {
    int i;

    for (i = 0; i < UKF_HIST; i++) {
	_navUkfData.posN[i] += deltaN;
	_navUkfData.posE[i] += deltaE;
	_navUkfData.posD[i] += deltaD;
    }
...
float AC_UKF::compassNormalize(float heading) {
    while (heading < 0.0f)
	heading += 360.0f;
...
void AC_UKF::navUkfNormalizeQuat(float *qr, float *q) {
    float norm;
...
void AC_UKF::crossVector3(float *vr, float *va, float *vb) {
    vr[0] = va[1] * vb[2] - vb[1] * va[2];
    vr[1] = va[2] * vb[0] - vb[2] * va[0];
    vr[2] = va[0] * vb[1] - vb[0] * va[1];
}

float AC_UKF::dotVector3(float *va, float *vb) {
    return va[0]*vb[0] + va[1]*vb[1] + va[2]*vb[2];
}

void AC_UKF::navUkfRotateVectorByQuat(float *vr, float *v, float *q) {
    float w, x, y, z;
...
void AC_UKF::navUkfRotateVectorByRevQuat(float *vr, float *v, float *q) {
    float qc[4];
...
void AC_UKF::navUkfMatrixExtractEuler(float *m, float *yaw, float *pitch, float *roll) {
...
void AC_UKF::navUkfQuatExtractEuler(float *q, float *yaw, float *pitch, float *roll) {
    float q0, q1, q2, q3;
...
void AC_UKF::navUkfRotateQuat(float *qOut, float *qIn, float *rate) {
    float q[4];
    float r[3];
...
void AC_UKF::navUkfTimeUpdate(float *in, float *noise, float *out, float *u, float dt, int n) {
    float tmp[3], acc[3];
    float rate[3];
    float mat3x3[3*3];
    float q[4];
...
void AC_UKF::navUkfRateUpdate(float *u, float *x, float *noise, float *y) {
    y[0] = -x[UKF_STATE_GYO_BIAS_X+(int)u[0]] + noise[0];
}

void AC_UKF::navUkfAccUpdate(float *u, float *x, float *noise, float *y) {
    navUkfRotateVectorByRevQuat(y, _v0a, &x[UKF_STATE_Q1]);
    y[0] += noise[0];
    y[1] += noise[1];
    y[2] += noise[2];
}

void AC_UKF::navUkfMagUpdate(float *u, float *x, float *noise, float *y) {
    navUkfRotateVectorByRevQuat(y, _v0m, &x[UKF_STATE_Q1]);
    y[0] += noise[0];
    y[1] += noise[1];
    y[2] += noise[2];
}
...
void AC_UKF::navUkfFinish(void) {
    navUkfNormalizeQuat(&UKF_Q1, &UKF_Q1);
    navUkfQuatExtractEuler(&UKF_Q1, &_yaw, &_pitch, &_roll);
    _yaw = compassNormalize(degrees(_yaw));
    //_navUkfData.pitch *= RAD_TO_DEG;
    //_navUkfData.roll *= RAD_TO_DEG;
    _pitch = degrees(_pitch);
    _roll = degrees(_roll);

    //    x' = x cos f - y sin f
    //    y' = y cos f + x sin f
    _yawCos = cosf(radians(_yaw));
    _yawSin = sinf(radians(_yaw));
}

void AC_UKF::navUkfInertialUpdate(float T) {
    float u[6];
    ...
    kf->srcdkfTimeUpdate(u, T);

    // store history
    /*_navUkfData.posN[navUkfData.navHistIndex] = UKF_POSN;
    _navUkfData.posE[navUkfData.navHistIndex] = UKF_POSE;
    _navUkfData.posD[navUkfData.navHistIndex] = UKF_POSD;

    _navUkfData.velN[navUkfData.navHistIndex] = UKF_VELN;
    _navUkfData.velE[navUkfData.navHistIndex] = UKF_VELE;
    _navUkfData.velD[navUkfData.navHistIndex] = UKF_VELD;*/

    //_navUkfData.navHistIndex = (_navUkfData.navHistIndex + 1) % UKF_HIST;
}

void AC_UKF::navUkfZeroRate(float rate, int axis) {
    ...
    kf->setMeasurementUpdate(&AC_UKF::navUkfRateUpdate);
    kf->srcdkfMeasurementUpdate(u, y, 1, 1, noise);
}
...
void AC_UKF::simDoAccUpdate(float accX, float accY, float accZ) {
    ...
    noise[0] = UKF_ACC_N + fabsf(GRAVITY_MSS - norm) * UKF_DIST_N;
    ...
    kf->setMeasurementUpdate(&AC_UKF::navUkfAccUpdate);
    kf->srcdkfMeasurementUpdate(0, y, 3, 3, noise);
}

void AC_UKF::simDoMagUpdate(float magX, float magY, float magZ) {
    float noise[3];        // measurement variance
    float y[3];            // measurement(s)
    float norm;
    ...
    kf->setMeasurementUpdate(&AC_UKF::navUkfMagUpdate);
    kf->srcdkfMeasurementUpdate(0, y, 3, 3, noise);
}
...
void AC_UKF::navUkfResetBias(void) {
    // acc bias
    UKF_ACC_BIAS_X = 0.0f;
    UKF_ACC_BIAS_Y = 0.0f;
    UKF_ACC_BIAS_Z = 0.0f;
...
void AC_UKF::navUkfInitState(void) {
    //uint32_t lastUpdate;
    float acc[3], mag[3];
...
//    UKF_PRES_ALT = navUkfPresToAlt(AQ_PRESSURE);
	...
	navUkfRotateVecByRevMatrix(estAcc, _v0a, m);

	// rotate mags to body frame of reference
	navUkfRotateVecByRevMatrix(estMag, _v0m, m);
...
void AC_UKF::navUkfNormalizeVec3(float *vr, float *v) {
    float norm;
