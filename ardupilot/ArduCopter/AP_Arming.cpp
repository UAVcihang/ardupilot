...
bool AP_Arming_Copter::board_voltage_checks(bool display_failure)
{
    ...
    if ((checks_to_perform == ARMING_CHECK_ALL) || (checks_to_perform & ARMING_CHECK_VOLTAGE)) {
        ...
        if (copter.battery.has_failsafed()/*exhausted(g.fs_batt_voltage, g.fs_batt_mah)*/) {
            if (display_failure) {
...
bool AP_Arming_Copter::arm_checks(bool display_failure, bool arming_from_gcs)
{
    ...
    // check battery voltage
    if ((checks_to_perform == ARMING_CHECK_ALL) || (checks_to_perform & ARMING_CHECK_VOLTAGE)) {
        if (copter.failsafe.battery || (!copter.ap.usb_connected && copter.battery.has_failsafed()/*exhausted(copter.g.fs_batt_voltage, copter.g.fs_batt_mah)*/)) {
            if (display_failure) {
