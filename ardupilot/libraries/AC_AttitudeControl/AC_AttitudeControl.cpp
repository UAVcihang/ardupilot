...
float AC_AttitudeControl::rate_target_to_motor_roll(float rate_actual_rads, float rate_target_rads)
{
    ...
    float output = get_rate_roll_pid().get_p() + integrator /*- kd * _foaw_roll.apply(rate_actual_rads)*/+ get_rate_roll_pid().get_d() + get_rate_roll_pid().get_ff(rate_target_rads);
...
float AC_AttitudeControl::rate_target_to_motor_pitch(float rate_actual_rads, float rate_target_rads)
{
    ...
    //float kd = get_rate_pitch_pid().kD();
    // Ensure that integrator can only be reduced if the output is saturated
    if (!_motors.limit.roll_pitch || ((is_positive(integrator) && is_negative(rate_error_rads)) || (is_negative(integrator) && is_positive(rate_error_rads)))) {
    ...
    // Compute output in range -1 ~ +1
    float output = get_rate_pitch_pid().get_p() + integrator /*- kd * _foaw_pitch.apply(rate_actual_rads)*/+ get_rate_pitch_pid().get_d() + get_rate_pitch_pid().get_ff(rate_target_rads);
...
float AC_AttitudeControl::rate_target_to_motor_yaw(float rate_actual_rads, float rate_target_rads)
{
    ...
    // Compute output in range -1 ~ +1
    float output = get_rate_yaw_pid().get_p() + integrator /*- kd * _foaw_yaw.apply(rate_actual_rads)*/+ get_rate_yaw_pid().get_d() + get_rate_yaw_pid().get_ff(rate_target_rads);
