...
void AC_PosControl::update_z_controller_new()
{
    ...
    // call z-axis position controller
    run_z_controller_new();
}

void AC_PosControl::run_z_controller_new()
{
    float curr_alt = _inav.get_altitude();
    ...
    // set input to PID
    _pid_accel_z.set_input_filter_all(_vel_error.z);

    // _pid_accel_z.set_desired_rate(_vel_error.z);
    ...
    //_accel_target.z = (p+i+d)*0.001f +_motors.get_throttle_hover();//_accel_desired.z;
    //_accel_target.z = -_accel_target.z;
...
void AC_PosControl::thrustToAttitude(float yaw)
{
	Vector3f body_x, body_y, body_z;

	_accel_target.x *= 0.01f;
	_accel_target.y *= 0.01f;

	body_z = -_accel_target.normalized();
	...
	Matrix3f rot_matrix(body_x.x, body_y.x, body_z.x, body_x.y, body_y.y, body_z.y, body_x.z, body_y.z, body_z.z);
	Vector3f euler;
	rot_matrix.to_euler(&euler.x, &euler.y, &euler.z);
	_pitch_target = euler.y * (18000.0f/M_PI);
	_roll_target = euler.x * (18000.0f/M_PI);

	_attitude_control.set_throttle_out(_accel_target.length(), true, POSCONTROL_THROTTLE_CUTOFF_FREQ);
	//for(int i = 0; i<3; i++)
