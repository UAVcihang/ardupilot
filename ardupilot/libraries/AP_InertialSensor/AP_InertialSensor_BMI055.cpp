                                ...
                                enum Rotation rotation)
{
	...
	//hal.console->printf("BMI055 sensor init\n");
    if (!sensor->init()) {
		//hal.console->printf("BMI055 sensor init error\n");
        delete sensor;
        return nullptr;
    }
...
bool AP_InertialSensor_BMI055::accel_init()
{
    ...
    uint8_t v;
	//bool flag = dev_accel->read_registers(REGA_BGW_CHIPID, &v, 1);
    if (!dev_accel->read_registers(REGA_BGW_CHIPID, &v, 1) || v != 0xFA) {
		/*if(flag)
			hal.console->printf("bmi acc 1:%d\n",v);*/
        goto failed;
    }

    if (!dev_accel->write_register(REGA_BGW_SOFTRESET, 0xB6)) {
		hal.console->printf("bmi acc 2\n");
        goto failed;
    }
