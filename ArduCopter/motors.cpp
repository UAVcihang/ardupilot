...
void Copter::arm_motors_check()
{
    ...
    int16_t tmp_roll = channel_roll->get_control_in();     //获得横滚通道数据
    int16_t tmp_pitch = channel_pitch->get_control_in();   //获得俯仰通道数据
    // full right
    if ((yaw_in > 4000)&&(tmp_roll<-4000)&&(tmp_pitch>4000))  //实现内八，解锁
    {
    ...
    else if ((yaw_in < -4000)&&(tmp_roll>4000)&&(tmp_pitch>4000)&& (arming_rudder == AP_Arming::ARMING_RUDDER_ARMDISARM)) //实现外八，上锁
    {
