...
void Copter::zigzag_run()
{
    ...
    case Zigzag_Auto:
    	if(zigzag_auto_complete_state && !zigzag_change_yaw/* && zigzag_waypoint_state.action*/){
            ...
            zigzag_auto_complete_state = false;
            wp_nav->set_fast_waypoint((g.u_turn > 0)?true:false);
...
void Copter::zigzag_auto_control()
{
    ...
    if (auto_yaw_mode == AUTO_YAW_HOLD) {
    	...
    	//attitude_control->input_euler_angle_roll_pitch_yaw(wp_nav->get_roll(), wp_nav->get_pitch(), (g.u_turn>0)?wp_nav->get_yaw():0, true/*, get_smoothing_gain()*/);
    }
...
void Copter::zigzag_set_destination(/*Location_Class& next_dest*/)
{
	...
	case Zigzag_None:
		...
		if(g.u_turn > 0 && (zigzag_waypoint_state.fly_direc < Zigzag_A2B )){
